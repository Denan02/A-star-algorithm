...
{
    std::cout << "========================================\n";
    std::cout << "A* ALGORITAM - KOMPLETNO TESTIRANJE\n";
    std::cout << "========================================\n\n";
    ...
    std::random_device rd;
    std::mt19937 gen(42); // Fixed seed za konzistentnost
    std::uniform_real_distribution<float> dis(1.0f, 10.0f);
    std::uniform_real_distribution<float> wall(0.0f, 1.0f);

    // Lambda za ispis putanje
    auto ispisiPutanju = [](const std::vector<int>& p, int cols) {
        std::cout << "  Duzina putanje: " << p.size() << " cvorova\n";
        if (p.size() > 10) {
            std::cout << "  Prvih 5: ";
            for (size_t i = 0; i < 5 && i < p.size(); i++) {
                int idx = p[i];
                std::cout << "(" << idx / cols << "," << idx % cols << ") ";
            }
            std::cout << "... Zadnjih 5: ";
            for (size_t i = (p.size() > 5 ? p.size() - 5 : 0); i < p.size(); i++) {
                int idx = p[i];
                std::cout << "(" << idx / cols << "," << idx % cols << ") ";
            }
            std::cout << "\n";
    ...
    // Lambda za generisanje mreže - OBJE verzije
    auto generisiMrezu = [&](int rows, int cols) -> std::pair<std::vector<std::vector<float>>, std::vector<float>> {
        // 2D verzija
        std::vector<std::vector<float>> cvorovi2d(rows * cols, std::vector<float>(8));
        // 1D flattened verzija
        ...
        return {cvorovi2d, cvorovi_flat};
    };

    // Testiranje za svaku dimenziju
    for (const auto& size : test_sizes) {
        int rows = size.first;
        int cols = size.second;

        std::cout << "\n========================================\n";
        std::cout << "TEST: " << rows << "x" << cols << " mreza\n";
        std::cout << "========================================\n";

        std::pair<int,int> start = {0, 0};
        std::pair<int,int> finish = {rows - 1, cols - 1};

        std::cout << "Generisanje mreze (" << rows << "x" << cols << ")...\n";
        auto [cvorovi2d, cvorovi_flat] = generisiMrezu(rows, cols);
        std::cout << "Mreza generisana!\n\n";
        ...
        // TEST 0: A_star (osnovna verzija sa 2D vectorom)
        std::cout << "[1/4] A_star (osnovna, 2D vector)...\n";
        try {
            auto t1 = std::chrono::high_resolution_clock::now();
            auto res = graf2d.A_star(start, finish);
            auto t2 = std::chrono::high_resolution_clock::now();
            ...
            if (res.first >= 0) {
                std::cout << "  ✓ Put pronadjen! Cijena: " << res.first << "\n";
                std::cout << "  Vrijeme: " << trajanje.count() << " s\n";
                ispisiPutanju(res.second, cols);
            } else {
                std::cout << "  Put nije pronadjen\n";
            }
        } catch (const std::exception& e) {
            std::cout << "  Greska: " << e.what() << "\n";
            rezultati.push_back({"A_star (2D)", -1, {}, 0, false});
        }

        // TEST 1: A_star_v2 (sa std::priority_queue i flattened vector)
        std::cout << "\n[2/4] A_star_v2 (flatten + std::priority_queue)...\n";
        try {
            auto t1 = std::chrono::high_resolution_clock::now();
            auto res = graf_flat.A_star_v2(start, finish);
            auto t2 = std::chrono::high_resolution_clock::now();
            std::chrono::duration<double> trajanje = t2 - t1;
            ...
            if (res.first >= 0) {
                std::cout << " Put pronađen! Cijena: " << res.first << "\n";
                std::cout << "  Vrijeme: " << trajanje.count() << " s\n";
                ispisiPutanju(res.second, cols);
            } else {
                std::cout << "  Put nije pronadjen\n";
            ...
            std::cout << "  Greska: " << e.what() << "\n";
            rezultati.push_back({"A_star_v2", -1, {}, 0, false});
        }

        // TEST 2: A_star_v3 (sa custom DaryHeap)
        std::cout << "\n[3/4] A_star_v3 (DaryHeap<4>)...\n";
        try {
            auto t1 = std::chrono::high_resolution_clock::now();
            auto res = graf_flat.A_star_v3(start, finish);
            auto t2 = std::chrono::high_resolution_clock::now();
            std::chrono::duration<double> trajanje = t2 - t1;
            ...
            if (res.first >= 0) {
                ...
                std::cout << "  Put nije pronađen\n";
            }
        } catch (const std::exception& e) {
            std::cout << "  Greska: " << e.what() << "\n";
            rezultati.push_back({"A_star_v3", -1, {}, 0, false});
        }

        // TEST 3: A_star_v4 (bidirectional paralelni)
        std::cout << "\n[4/4] A_star_v4 (Bidirectional + OpenMP)...\n";
        try {
            auto t1 = std::chrono::high_resolution_clock::now();
            auto res = graf_flat.A_star_v4(start, finish);
            auto t2 = std::chrono::high_resolution_clock::now();
            std::chrono::duration<double> trajanje = t2 - t1;
            ...
            std::cout << "  Greska: " << e.what() << "\n";
            rezultati.push_back({"A_star_v4", -1, {}, 0, false});
        }

        // UPOREDNI REZULTATI
        std::cout << "\n----------------------------------------\n";
        std::cout << "UPOREDNI REZULTATI (" << rows << "x" << cols << ")\n";
        std::cout << "----------------------------------------\n";
        std::cout << std::left << std::setw(20) << "Metoda"
                  << std::setw(15) << "Vrijeme (s)"
                  << std::setw(15) << "Cijena puta"
                  << std::setw(15) << "Duzina puta" << "\n";
        std::cout << "----------------------------------------\n";

        double najbolje_vrijeme = std::numeric_limits<double>::max();
        std::string najbolja_metoda;

        for (const auto& r : rezultati) {
            if (r.uspjeh) {
                std::cout << std::left << std::setw(20) << r.naziv
                          << std::setw(15) << std::fixed << std::setprecision(4) << r.vrijeme
                          << std::setw(15) << std::fixed << std::setprecision(2) << r.cijena
                          << std::setw(15) << r.putanja.size() << "\n";

                if (r.vrijeme < najbolje_vrijeme) {
                    najbolje_vrijeme = r.vrijeme;
                    najbolja_metoda = r.naziv;
                }
            } else {
                std::cout << std::left << std::setw(20) << r.naziv
                          << std::setw(15) << "NEUSPJESNO" << "\n";
            }
        }

        // SPEEDUP ANALIZA - uporedi sa osnovnom verzijom
        if (rezultati.size() > 1 && rezultati[0].uspjeh) {
            std::cout << "\n--- Speedup analiza (vs " << rezultati[0].naziv << ") ---\n";
            double baseline = rezultati[0].vrijeme;
            for (size_t i = 1; i < rezultati.size(); i++) {
                if (rezultati[i].uspjeh) {
                    double speedup = baseline / rezultati[i].vrijeme;
                    std::cout << rezultati[i].naziv << ": "
                              << std::fixed << std::setprecision(2)
                              << speedup << "x ";
                    if (speedup > 1.0) std::cout << "(BRZE )";
                    else if (speedup < 1.0) std::cout << "(SPORIJE )";
                    else std::cout << "(ISTO)";
                    std::cout << "\n";
                }
            }
        ...
        std::cout << "\n Najbolja metoda: " << najbolja_metoda
                  << " (" << najbolje_vrijeme << " s)\n";

        // VERIFIKACIJA REZULTATA - provjeri da li sve cijene odgovaraju
        std::cout << "\n--- Verifikacija korektnosti ---\n";
        if (rezultati.size() > 1 && rezultati[0].uspjeh) {
            float referentna_cijena = rezultati[0].cijena;
            bool sve_ok = true;

            for (size_t i = 1; i < rezultati.size(); i++) {
                if (rezultati[i].uspjeh) {
                    float razlika = std::abs(rezultati[i].cijena - referentna_cijena);
                    if (razlika > 0.01f) {
                        std::cout << "  " << rezultati[i].naziv << ": cijena "
                                  << rezultati[i].cijena << " vs " << referentna_cijena
                                  << " (razlika: " << razlika << ")\n";
                        sve_ok = false;
                    } else {
                        std::cout << " " << rezultati[i].naziv << ": cijena OK ("
                                  << rezultati[i].cijena << ")\n";
                    }
                }
            }
            ...
            if (sve_ok) {
                std::cout << "SVE metode vratile istu optimalnu cijenu!\n";
            } else {
                std::cout << " UPOZORENJE: Različite cijene - neki algoritam je NEISPRAVAN!\n";
            }
        }
    }

    std::cout << "\n========================================\n";
    std::cout << "TESTIRANJE ZAVRSENO\n";
    std::cout << "========================================\n";
